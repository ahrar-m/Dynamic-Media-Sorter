...
var model2 = (game, options = {}) => {
  ...
  return teamRatings.map((iTeamRating, i) => {
    const [iMu, iSigmaSq, iTeam, iRank] = iTeamRating;
    const { omega: iOmega, delta: iDelta } = teamRatings.filter((_, q) => q !== i).reduce(
...
var model4 = (game, options = {}) => {
  ...
  return teamRatings.map((iTeamRating, i) => {
    const [iMu, iSigmaSq, iTeam, iRank] = iTeamRating;
    const { omega: iOmega, delta: iDelta } = teamRatings.filter((_, q) => i !== q).reduce(
      (acc, [qMu, qSigmaSq, _qTeam, qRank]) => {
        const ciq = Math.sqrt(iSigmaSq + qSigmaSq + TWOBETASQ);
